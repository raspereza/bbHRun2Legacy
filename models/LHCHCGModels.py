...
class Kappas(LHCHCGBaseModel):
    ...
    def __init__(self,resolved=True,addInterference=False,BRU=True,addInvisible=False,addUndet=False,addWidth=False,addKappaC=False,custodial=False):
        LHCHCGBaseModel.__init__(self) # not using 'super(x,self).__init__' since I don't understand it
        self.doBRU = BRU
        self.resolved = resolved
        self.addInterference = addInterference
    ...
    def doParametersOfInterest(self):
        ...
        self.modelBuilder.doVar("kappa_t[1,-5.0,5.0]")
        ...
        if self.addInterference: 
            self.modelBuilder.out.var("kappa_tau").setConstant(True)
            self.modelBuilder.out.var("kappa_Z").setConstant(True)
            self.modelBuilder.out.var("kappa_W").setConstant(True)
            self.modelBuilder.out.var("kappa_mu").setConstant(True)
    ...
    def getHiggsSignalYieldScale(self,production,decay,energy):
        ...
        self.modelBuilder.factory_('expr::kbktinterference_scale("-1*@0*@1", kappa_b,kappa_t)')
        self.modelBuilder.out.function("kbktinterference_scale")
        if self.modelBuilder.out.function(name) == None:
            ...
            elif self.addInterference and production=="intH": XSscal = ("@0", "kbktinterference_scale")
...
K1Int = Kappas(resolved=True,addInterference=True)
